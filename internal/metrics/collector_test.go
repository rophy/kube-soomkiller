...
func TestFindPodCgroups(t *testing.T) {
	// Create temp directory structure mimicking cgroup hierarchy
	tmpDir := t.TempDir()

	// Create kubepods.slice structure
	paths := []string{
		"kubepods.slice/kubepods-burstable.slice/kubepods-burstable-pod123.slice/cri-containerd-abc123def456.scope",
		"kubepods.slice/kubepods-burstable.slice/kubepods-burstable-pod123.slice/cri-containerd-sandbox111.scope",
		"kubepods.slice/kubepods-besteffort.slice/kubepods-besteffort-pod456.slice/cri-containerd-xyz789.scope",
		"kubepods.slice/kubepods-guaranteed.slice/kubepods-guaranteed-pod789.slice/cri-containerd-ghi012.scope",
	}
	...
	if len(cgroups) != 4 {
		t.Errorf("FindPodCgroups() returned %d cgroups, want 4", len(cgroups))
	}
	...
	for _, prefix := range expectedPrefixes {
		...
		for _, cg := range cgroups {
			if filepath.HasPrefix(cg, prefix) {
				found = true
				break
			}
		}
		if !found {
			t.Errorf("Expected cgroup with prefix %s not found", prefix)
		}
	}
}

func TestFindPodCgroups_NoKubepods(t *testing.T) {
	tmpDir := t.TempDir()
...
func TestFindPodCgroups_EmptyKubepods(t *testing.T) {
	tmpDir := t.TempDir()

	// Create empty kubepods.slice
	...
	if len(cgroups) != 0 {
		t.Errorf("FindPodCgroups() returned %d cgroups, want 0", len(cgroups))
	}
}

func TestFindPodCgroups_IgnoresNonContainerDirs(t *testing.T) {
	tmpDir := t.TempDir()

	// Create mix of container and non-container directories
	paths := []string{
		...
		"kubepods.slice/kubepods-burstable.slice/kubepods-burstable-pod123.slice/system.slice",
	...
	// Should only find cri-containerd-* directories
	if len(cgroups) != 1 {
